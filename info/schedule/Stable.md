...
* The properties [`BackgroundColor`]({{site.info}}api/WebTwain_Viewer.html#backgroundcolor) and [`SelectionImageBorderColor`]({{site.info}}api/WebTwain_Viewer.html#selectionimagebordercolor) will always return a string that represents the style (colour, border, etc.) even if you set them with numbers. For example, `BackgroundColor` returns "#000032" if you set it to `50` and `SelectionImageBorderColor` returns "1px solid #000032" when you set it to `50`.

* The properties [`IfFitWindow`]({{site.info}}api/WebTwain_Viewer.html#iffitwindow) and [`FitWindowType`]({{site.info}}api/WebTwain_Viewer.html#fitwindowtype) are now write-only as their actual values may not be correct when you read them. As these properties are deprecated, use [`fitWindow()`]({{site.info}}api/WebTwain_Viewer.html#fitwindow) instead.

* Added a new parameter `fill` to the method [`play()`]({{site.info}}api/Addon_Camera.html#play).
